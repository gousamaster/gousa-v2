...
export async function GET(){
  try{
    ...
    const rows=await db.$queryRaw<Array<{
      ...
      participantes:number;resultado:string|null;registradoAt:Date|null;horasDesdeCita:number;diaOffset:number;recordatorioEnviado:boolean|null;
    }>>`
    ...
    const entrevistas=rows.map(r=>({
      ...
      diaOffset:Number(r.diaOffset),
      recordatorioEnviado:r.recordatorioEnviado===true,
    ...
    return NextResponse.json({error:"No se pudieron cargar las entrevistas próximas"},{status:500});
  }
}
