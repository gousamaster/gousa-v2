...
export async function GET(){
  try{
    await Promise.all([ensureCitaResultadoConsularSchema(),ensureEntrevistaRecordatorioSchema()]);
    const session=await auth.api.getSession({headers:await headers()});
    if(!session?.user?.id)return NextResponse.json({error:"No autorizado"},{status:401});
    const rows=await db.$queryRaw<Array<{
      id:string;fechaHora:Date;lugar:string|null;tipoCita:string;cliente:string|null;grupoFamiliar:string|null;participantes:number;resultado:string|null;registradoAt:Date|null;horasDesdeCita:number;diaOffset:number;recordatorioEnviado:boolean|null;regionNombre:string|null;
    }>>`
    ...
    const regionAgenda=(regionNombre:string|null,lugar:string|null)=>{
      const base=`${regionNombre??""} ${lugar??""}`.toLowerCase();
      if(base.includes("cochabamba"))return "COCHABAMBA";
      if(base.includes("la paz")||base.includes("embajada"))return "LA_PAZ";
      return "INTERIOR";
    };
    const entrevistas=rows.map(r=>({
      ...r,participantes:Number(r.participantes),horasDesdeCita:Number(r.horasDesdeCita),diaOffset:Number(r.diaOffset),recordatorioEnviado:r.recordatorioEnviado===true,region:regionAgenda(r.regionNombre,r.lugar),
      estadoResultado:r.resultado?r.resultado:Number(r.diaOffset)>0?"PROXIMA":Number(r.horasDesdeCita)>=6?"ATRASADO":Number(r.horasDesdeCita)>=0?"PENDIENTE":"PROXIMA",
    }));
