...
export async function GET() {
  try {
    ...
    const clasificar=(nombre:string,codigo:string|null)=>{
      const valor=`${nombre} ${codigo??""}`.toLowerCase();
      if(valor.includes("simulacr")) return "SIMULACRO";
      return "ASESORIA";
    };
    ...
    return NextResponse.json({
      simulacros: agenda.filter(a=>a.categoria==="SIMULACRO"),
      asesorias: agenda.filter(a=>a.categoria==="ASESORIA"),
    });
  } catch (error) {
    console.error("agenda-hoy GET", error);
    return NextResponse.json({ error: "No se pudo cargar la agenda de hoy" }, { status: 500 });
  }
}
