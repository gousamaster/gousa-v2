...
function icsDate(fecha:string,hora:string){const d=new Date(`${fecha}T${hora}:00-04:00`);return d.toISOString().replace(/[-:]/g,"").replace(/\.\d{3}Z$/,"Z");}
...
async function enviarInvitacionEquipo(args:{titulo:string;detalle:string|null;fecha:string;horaInicio:string;horaFin:string;creadoPor:string}){
  ...
  const r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({from,to:destinatarios,subject:`NEXUS · ${args.titulo}`,html:`<div style="font-family:Arial,sans-serif;line-height:1.5"><h2>${args.titulo}</h2><p><strong>Fecha:</strong> ${etiquetaFecha}</p><p><strong>Hora:</strong> ${args.horaInicio} - ${args.horaFin}</p>${args.detalle?`<p><strong>Detalle:</strong> ${args.detalle}</p>`:""}<p>Actividad creada por ${args.creadoPor} en NEXUS Dashboard Time.</p></div>`,attachments:[{filename:"actividad-nexus.ics",content:Buffer.from(ics,"utf8").toString("base64")}]})});
...
export async function POST(request:Request){
  try{
    ...
    if(!body?.titulo||!body?.fechaObjetivo||!body?.horaInicio)return NextResponse.json({error:"Título, fecha y hora son obligatorios"},{status:400});
    const categoria=String(body.categoria??"OTRO"),esEquipo=CATEGORIAS_EQUIPO.has(categoria);
    if(esEquipo&&!body.horaFin)return NextResponse.json({error:"Hora fin es obligatoria para reuniones y capacitaciones"},{status:400});
    if(esEquipo&&String(body.horaFin)<=String(body.horaInicio))return NextResponse.json({error:"La hora fin debe ser posterior a la hora de inicio"},{status:400});
    const id=randomUUID(),fecha=new Date(`${body.fechaObjetivo}T12:00:00-04:00`),enviarCorreoEquipo=esEquipo&&body.enviarCorreoEquipo!==false;
    await db.$executeRaw`INSERT INTO "nexus_pendiente" ("id","titulo","detalle","categoria","fechaObjetivo","horaInicio","horaFin","enviarCorreoEquipo","prospectoId","clienteId","asignadoAId","creadoPorId") VALUES (${id},${String(body.titulo)},${body.detalle?String(body.detalle):null},${categoria},${fecha},${String(body.horaInicio)},${body.horaFin?String(body.horaFin):null},${enviarCorreoEquipo},${body.prospectoId??null},${body.clienteId??null},${esEquipo?null:(body.asignadoAId??null)},${session.user.id})`;
    let correoEstado:string|null=null;
    if(enviarCorreoEquipo){const envio=await enviarInvitacionEquipo({titulo:String(body.titulo),detalle:body.detalle?String(body.detalle):null,fecha:String(body.fechaObjetivo),horaInicio:String(body.horaInicio),horaFin:String(body.horaFin),creadoPor:session.user.name??"Equipo GO USA"});correoEstado=envio.estado;await db.$executeRaw`UPDATE "nexus_pendiente" SET "correoEstado"=${envio.estado},"correoEnviadoAt"=${envio.enviado?new Date():null},"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id}`;}
