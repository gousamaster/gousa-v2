...
export async function GET(
  ...
  try {
    ...
    const [seguimientos, responsables] = await Promise.all([
      db.$queryRaw<SeguimientoRow[]>`
        SELECT
          ...
          s."estado" AS "estado",
          s."notas" AS "notas",
          s."completado_at" AS "completadoAt",
        ...
        ORDER BY
          ...
          CASE WHEN s."estado" = 'PENDIENTE' THEN s."programado_at" END ASC,
          s."created_at" DESC
        LIMIT 30
      `,
      db.user.findMany({
    ...
    return NextResponse.json(
      { error: "No se pudo cargar el seguimiento comercial" },
      { status: 500 },
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const responsableId =
      typeof body.responsableId === "string" && body.responsableId.trim()
        ? body.responsableId.trim()
        : session.user.id;
    const notas =
      typeof body.notas === "string" && body.notas.trim() ? body.notas.trim() : null;
    const programadoAt = new Date(body.programadoAt);

    if (!accion || Number.isNaN(programadoAt.getTime())) {
      return NextResponse.json(
        { error: "Acción y fecha/hora de seguimiento son obligatorias" },
        { status: 400 },
      );
    }
    ...
    const [prospecto, responsable] = await Promise.all([
      ...
      db.user.findFirst({
        where: { id: responsableId, status: "ACTIVE" },
        select: { id: true },
      }),
    ]);
    ...
    if (!prospecto) {
      return NextResponse.json(
        { error: "El prospecto no existe o ya fue convertido" },
        { status: 400 },
      );
    }
    ...
    await db.$executeRaw`
      INSERT INTO "prospecto_seguimiento" (
        "id",
        "prospecto_id",
        "responsable_id",
        "creado_por_id",
    ...
    if (body.actualizarEstado !== false) {
    ...
    return NextResponse.json(
      { error: "No se pudo programar el seguimiento" },
      { status: 500 },
    );
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    const body = await request.json();
    const seguimientoId =
      typeof body.seguimientoId === "string" ? body.seguimientoId.trim() : "";
    ...
    if (!seguimientoId || !["COMPLETADO", "CANCELADO"].includes(estado)) {
      return NextResponse.json({ error: "Actualización de seguimiento no válida" }, { status: 400 });
    }
    ...
    const updated = await db.$executeRaw`
      ...
      WHERE "id" = ${seguimientoId}
        AND "prospecto_id" = ${prospectoId}
        AND "estado" = 'PENDIENTE'
    `;

    if (updated === 0) {
      return NextResponse.json(
        { error: "Seguimiento no encontrado o ya cerrado" },
        { status: 404 },
      );
    }
