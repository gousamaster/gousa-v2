...
export async function GET(_request:Request,{params}:{params:Promise<{clienteId:string}>}){
  try{
    ...
    const cliente=await db.cliente.findFirst({where:{id:clienteId,deletedAt:null},select:{id:true}});
...
export async function POST(request:Request,{params}:{params:Promise<{clienteId:string}>}){
  try{
    ...
    await db.$executeRaw`
      ...
      VALUES (${docId},${clienteId},${tipo},${archivo.name},${archivo.type},${archivo.size},${contenido},${session.user.id},NOW(),NOW())
