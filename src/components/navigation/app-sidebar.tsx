...
import { AppWindowIcon, BarChart3Icon, CalendarIcon, CommandIcon, GaugeIcon, NotebookIcon, SettingsIcon, UserPlusIcon, UsersIcon, PlaneIcon, Building2Icon } from "lucide-react";
...
export function AppSidebar({role,...props}:React.ComponentProps<typeof Sidebar>&{role?:string|null}){
 const navItems=isActivacionVentas(role)?data.navMain.filter((item)=>["/dashboard","/nexus-score","/prospectos","/servicios"].includes(item.url)):data.navMain;
