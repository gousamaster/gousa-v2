  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent><Skeleton className="mb-4 h-10 w-full" /><div className="space-y-2">{Array.from({ length: 8 }).map((_, i) => <Skeleton key={`skeleton-${i}`} className="h-16 w-full" />)}</div></CardContent>
      </Card>
    </div>
  );
}
...
export function ClientsContainer() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [clientesResult, regionesResult] = await Promise.all([obtenerTodosLosClientes(), obtenerTodasLasRegiones()]);
  ...
  if (isLoading) return <div className="flex-1 space-y-6 p-8 pt-6"><div><h2 className="text-3xl font-bold tracking-tight">Clientes</h2><p className="text-muted-foreground">Gestiona clientes, afiliados y sus datos</p></div><LoadingSkeleton /></div>;

  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      <div><h2 className="text-3xl font-bold tracking-tight">Clientes</h2><p className="text-muted-foreground">Flujo NEXUS: Prospecto → Cliente → Afiliado</p></div>
