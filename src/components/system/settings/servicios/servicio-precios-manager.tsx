// src/components/system/settings/servicios/servicio-precios-manager.tsx
...
import {
  agregarPrecioAServicio,
  eliminarPrecioServicio,
  obtenerPreciosPorServicio,
  togglePrecioServicioActivo,
...
interface ServicioPreciosManagerProps {
  open: boolean;
...
export function ServicioPreciosManager({
  open,
  onOpenChange,
  servicio,
  regiones,
  onSuccess,
}: ServicioPreciosManagerProps) {
  const [precios, setPrecios] = useState<ServicioPrecioDetalle[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showAddForm, setShowAddForm] = useState(false);
  const [precioToDelete, setPrecioToDelete] =
    useState<ServicioPrecioDetalle | null>(null);
  ...
  const loadPrecios = async () => {
    setIsLoading(true);
    try {
      const result = await obtenerPreciosPorServicio(servicio.id);
  ...
  async function onSubmit(values: PrecioFormValues) {
    ...
    try {
      const result = await agregarPrecioAServicio({
  ...
  const handleTogglePrecio = async (precio: ServicioPrecioDetalle) => {
    const result = await togglePrecioServicioActivo(precio.id);
    if (result.success) {
      toast.success(
  ...
  const handleDeletePrecio = (precio: ServicioPrecioDetalle) => {
  ...
  const confirmDelete = async () => {
    ...
    const result = await eliminarPrecioServicio(precioToDelete.id);
    if (result.success) {
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Gestionar Precios - {servicio.nombre}</DialogTitle>
            <DialogDescription>
              Configura los precios del servicio por región
            </DialogDescription>
          </DialogHeader>
