...
import { ChinaDocumentosChecklist } from "@/components/system/tramites/china-documentos-checklist";
...
export function ChinaOperativoCard({tramiteId}:{tramiteId:string}){
  ...
  return <div className="rounded-xl border border-red-200 bg-red-50/40 p-4 space-y-5">
    ...
    <div className="rounded-lg border bg-background/80 p-3 space-y-4">
      <div><p className="font-semibold text-sm">Acceso a cuenta Visa China</p><p className="mt-1 text-xs text-muted-foreground">Datos de acceso utilizados para administrar y monitorear la solicitud consular China.</p></div>
      <div className="space-y-2">
        <Label>Correo electrónico registrado para la cuenta</Label>
        <Input type="email" value={data.correoCuenta??""} onChange={e=>{set("correoCuenta",e.target.value||null);setUsarCorreoBase(Boolean(data.correoBaseCliente && e.target.value===data.correoBaseCliente))}} placeholder="correo@ejemplo.com"/>
        <label className="flex items-center gap-2 text-sm"><Checkbox checked={usarCorreoBase} disabled={!data.correoBaseCliente} onCheckedChange={v=>cambiarUsoCorreoBase(v===true)}/>Usar mismo correo de base de datos</label>
      ...
      <div className="space-y-2">
        ...
        <div className="relative"><Input type={mostrarContrasena?"text":"password"} value={data.contrasenaCuenta??""} onChange={e=>set("contrasenaCuenta",e.target.value||null)} placeholder="Contraseña de la cuenta consular" className="pr-10"/><button type="button" onClick={()=>setMostrarContrasena(v=>!v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground" aria-label={mostrarContrasena?"Ocultar contraseña":"Ver contraseña"}>{mostrarContrasena?<EyeOff className="h-4 w-4"/>:<Eye className="h-4 w-4"/>}</button></div>
      </div>
    </div>

    <label className="flex items-center gap-2 text-sm"><Checkbox checked={data.formularioIniciado} onCheckedChange={v=>set("formularioIniciado",v===true)}/>Formulario iniciado/completado</label>

    <ChinaDocumentosChecklist tramiteId={tramiteId}/>
