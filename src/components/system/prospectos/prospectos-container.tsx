...
function prioridadLabel(value: string) {
  const labels: Record<string, string> = { ALTA: "Prioridad alta", MEDIA: "Prioridad media", BAJA: "Prioridad baja", SIN_EVALUAR: "Sin evaluar", CONVERTIDO: "Convertido" };
...
export function ProspectosContainer() {
  ...
  const loadProspectos = async () => {
    try { setLoading(true); const response = await fetch("/api/prospectos", { cache: "no-store" }); const data = await response.json();
      if (!response.ok) throw new Error(data.error || "No se pudieron cargar los prospectos"); setProspectos(data.prospectos ?? []);
    } catch (err) { console.error(err); setError("No se pudieron cargar los prospectos"); } finally { setLoading(false); }
  };
  useEffect(() => { loadProspectos(); }, []);

  const prospectosOrdenados = useMemo(() => {
    const activos = prospectos.filter(p => !p.convertido); const convertidos = prospectos.filter(p => p.convertido);
    const ordenar = (items: Prospecto[]) => [...items].sort((a,b) => { const sa=a.scorePreliminar??-1, sb=b.scorePreliminar??-1; if(sa!==sb)return sb-sa; return new Date(b.createdAt).getTime()-new Date(a.createdAt).getTime(); });
    const lista=[...ordenar(activos),...ordenar(convertidos)]; if(filtro==="TODOS") return lista; return lista.filter(p=>prioridadScore(p)===filtro);
  }, [prospectos,filtro]);
  ...
  return <div className="flex-1 space-y-6 p-8 pt-6">
    ...
    <Card><CardHeader className="gap-4"><div><CardTitle>Prospectos registrados</CardTitle><p className="mt-1 text-sm text-muted-foreground">Los prospectos activos se ordenan por Score NEXUS para facilitar el seguimiento comercial.</p></div><div className="flex flex-wrap gap-2">{[["TODOS","Todos"],["ALTA","Alta"],["MEDIA","Media"],["BAJA","Baja"],["SIN_EVALUAR","Sin evaluar"],["CONVERTIDO","Convertidos"]].map(([value,label])=><Button key={value} type="button" size="sm" variant={filtro===value?"default":"outline"} onClick={()=>setFiltro(value)}>{label}</Button>)}</div></CardHeader><CardContent>
      {loading?<p className="text-sm text-muted-foreground">Cargando prospectos...</p>:prospectosOrdenados.length===0?<p className="text-sm text-muted-foreground">{prospectos.length===0?"Todavía no hay prospectos registrados.":"No hay prospectos en este filtro."}</p>:<div className="space-y-3">{prospectosOrdenados.map(prospecto=>{const prioridad=prioridadScore(prospecto);return <div key={prospecto.id} onClick={()=>router.push(`/prospectos/${prospecto.id}`)} className="flex cursor-pointer flex-col gap-3 rounded-lg border p-4 transition-colors hover:bg-muted/50 md:flex-row md:items-center md:justify-between"><div><p className="font-medium">{prospecto.nombres} {prospecto.apellidos??""}</p><p className="text-sm text-muted-foreground">{prospecto.telefono}{prospecto.email?` · ${prospecto.email}`:""}</p><p className="text-sm text-muted-foreground">Interés: {prospecto.interes||"Sin definir"} · Fuente: {etiquetaOrigenProspecto(prospecto.origen)}</p><p className="text-xs text-muted-foreground">Registrado por: {prospecto.creadoPor?.name||"Sin identificar"}</p></div><div className="flex flex-wrap items-center gap-2 text-sm md:justify-end">{prospecto.scorePreliminar!==null?<span className="rounded-full border px-3 py-1 font-medium">Score {prospecto.scorePreliminar}%</span>:<span className="rounded-full border border-dashed px-3 py-1 text-muted-foreground">Score pendiente</span>}<span className="rounded-full border px-3 py-1">{prioridadLabel(prioridad)}</span><span className="rounded-full border px-3 py-1">{prospecto.estado}</span></div></div>})}</div>}
    </CardContent></Card>
  </div>;
}
