...
export function ProspectoDetalle({ prospectoId }: { prospectoId: string }) {
  ...
  return <div className="flex-1 space-y-6 p-8 pt-6">
    ...
    {editing && editForm ? <Card><CardHeader><CardTitle>Editar prospecto</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="space-y-2"><Label>Origen</Label><Input value={editForm.origen ?? ""} onChange={e => updateField("origen", e.target.value)} /></div><div className="space-y-2"><Label>Interés</Label><Input value={editForm.interes ?? ""} onChange={e => updateField("interes", e.target.value)} /></div>
