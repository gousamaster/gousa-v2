...
export function CumpleanosTimeCard(){
  ...
  return <Card className="border-violet-200 shadow-sm">
    <CardHeader className="pb-3">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Recordatorio de fidelización: felicita a los clientes que cumplen años hoy.</p>
        </div>
    ...
    <CardContent>
      ...
      {loading&&cumpleanos.length===0?<p className="text-sm text-muted-foreground">Cargando cumpleaños...</p>:cumpleanos.length===0?<div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">No hay cumpleaños registrados para hoy.</div>:<div className="space-y-2">{cumpleanos.map(c=><div key={c.id} className="flex flex-col gap-1 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-medium">{c.nombreCompleto}</div><div className="text-xs text-muted-foreground">Cumple {c.edad} años</div></div><div className="text-xs text-muted-foreground">Cliente activo · Fidelización</div></div>)}</div>}
    </CardContent>
  </Card>;
}
