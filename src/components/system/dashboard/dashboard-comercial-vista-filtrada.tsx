...
export async function DashboardComercialVistaFiltrada({ filtros }: { filtros: Filtros }) {
  ...
  const metricas=[["Prospectos",total,"Resultado del filtro aplicado",UsersRound],["Convertidos",convertidos,`${pct(convertidos,total)}% de conversión`,CheckCircle2],["Prioridad alta",n(r?.altaPrioridad),"Score NEXUS ≥75",Target],["Perdidos",perdidos,`${pct(perdidos,total)}% del universo filtrado`,AlertTriangle],["Score promedio",scorePromedio==null?"—":`${scorePromedio}%`,"Solo prospectos evaluados",Gauge]] as const;
