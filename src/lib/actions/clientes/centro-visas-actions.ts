...
import { decryptCredential, getClienteCentroVisas, upsertClienteCentroVisas } from "@/lib/cliente-centro-visas";
...
export async function revelarPasswordCentroVisasCliente(clienteId:string){
  const session=await auth.api.getSession({headers:await headers()});
  if(!session?.user?.id)return {success:false,error:"No autorizado"} as const;
  try{
    const row=await getClienteCentroVisas(clienteId);
    if(!row?.passwordEncrypted)return {success:false,error:"No hay contraseña registrada"} as const;
    return {success:true,password:decryptCredential(row.passwordEncrypted)} as const;
  }catch(error){
    console.error("revelarPasswordCentroVisasCliente",error);
    return {success:false,error:"No se pudo mostrar la contraseña"} as const;
  }
}
