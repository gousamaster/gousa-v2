...
export async function ensureVuelosSchema() {
  ...
  await db.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "orden_cotizacion_vuelo" ("id" TEXT NOT NULL PRIMARY KEY,"numeroOrden" TEXT NOT NULL UNIQUE,"prospectoId" TEXT REFERENCES "prospecto"("id") ON DELETE RESTRICT ON UPDATE CASCADE,"clienteId" TEXT REFERENCES "cliente"("id") ON DELETE RESTRICT ON UPDATE CASCADE,"tipoViaje" TEXT NOT NULL DEFAULT 'IDA_VUELTA',"origen" VARCHAR(3) NOT NULL,"destino" VARCHAR(3) NOT NULL,"fechaIda" TIMESTAMP(3) NOT NULL,"fechaRetorno" TIMESTAMP(3),"tramos" JSONB,"flexibilidad" BOOLEAN NOT NULL DEFAULT false,"equipaje" BOOLEAN NOT NULL DEFAULT false,"observaciones" TEXT,"estado" TEXT NOT NULL DEFAULT 'PENDIENTE',"creadoPorId" TEXT NOT NULL REFERENCES "user"("id") ON DELETE RESTRICT ON UPDATE CASCADE,"createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"despachadoAt" TIMESTAMP(3),CHECK ("origen" <> "destino"))`);
