...
export async function ensureVuelosSchema() {
  ...
  await db.$executeRawUnsafe(`
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'orden_cotizacion_vuelo_clienteId_fkey') THEN
        ALTER TABLE "orden_cotizacion_vuelo"
        ADD CONSTRAINT "orden_cotizacion_vuelo_clienteId_fkey"
