...
export async function ensureRentACarSchema() {
  ...
  await db.$executeRawUnsafe(`ALTER TABLE "orden_cotizacion_rentacar" ADD COLUMN IF NOT EXISTS "montoVenta" NUMERIC(12,2)`);
  await db.$executeRawUnsafe(`ALTER TABLE "orden_cotizacion_rentacar" ADD COLUMN IF NOT EXISTS "monedaVenta" TEXT`);
  await db.$executeRawUnsafe(`ALTER TABLE "orden_cotizacion_rentacar" ADD COLUMN IF NOT EXISTS "metodoPago" TEXT`);
  await db.$executeRawUnsafe(`ALTER TABLE "orden_cotizacion_rentacar" ADD COLUMN IF NOT EXISTS "fechaEmision" TIMESTAMP(3)`);
  await db.$executeRawUnsafe(`ALTER TABLE "orden_cotizacion_rentacar" ADD COLUMN IF NOT EXISTS "observacionPago" TEXT`);
