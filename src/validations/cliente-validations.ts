...
export const updateClienteSchema = z.object({
  nombres: z.string().min(2).max(255).trim().optional(), apellidos: z.string().min(2).max(255).trim().optional(), tipoCliente: z.nativeEnum(TipoCliente).optional(), fechaNacimiento: optionalDateSchema,
  lugarNacimiento: z.string().max(255).trim().optional().nullable(), nacionalidad: z.string().max(100).trim().optional().nullable(), numeroCi: z.string().max(50).trim().optional().nullable(), numeroPasaporte: z.string().max(50).trim().optional().nullable(),
  email: z.string().email("Email inválido").max(255).trim().optional().nullable().or(z.literal("")), telefonoCelular: z.string().max(50).trim().optional().nullable(), regionId: z.string().optional(), activo: z.boolean().optional(),
});

export const createClienteDatosPersonalesSchema = z.object({
  pasaporteFechaEmision: optionalDateSchema, pasaporteFechaExpiracion: optionalDateSchema, facebook: z.string().max(255).trim().optional().nullable(), instagram: z.string().max(255).trim().optional().nullable(), direccionDomicilio: z.string().max(1000).trim().optional().nullable(), estadoCivil: z.string().max(50).trim().optional().nullable(), profesion: z.string().max(255).trim().optional().nullable(), nombrePadre: z.string().max(255).trim().optional().nullable(), fechaNacimientoPadre: optionalDateSchema, nombreMadre: z.string().max(255).trim().optional().nullable(), fechaNacimientoMadre: optionalDateSchema,
});
...
export const centroVisasSchema=z.object({
  mismoCorreo:z.boolean().default(true),
  email:z.string().email("Email de Centro de Visas inválido").max(255).trim().optional().nullable().or(z.literal("")),
  password:z.string().max(255).optional().nullable().or(z.literal("")),
}).optional();
